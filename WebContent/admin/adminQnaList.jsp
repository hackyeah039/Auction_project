...
<div id="paging">
	...
	<c:choose>
		<c:when test="${startPage>3}">
			<li class="page-item">
			<a href="${cp}/board/qnalist.jh?pageNum=${startPage-3}&field=${field}&keyword=${keyword}"
	...
	<c:forEach var="i" begin="${startPage }" end="${endPage }">
		<c:choose>
			<c:when test="${pageNum==i }">
				<li class="page-item">
				<a href="${cp}/board/qnalist.jh?pageNum=${i}&field=${field}&keyword=${keyword}" class="page-link">
				${i}</a></li>
			</c:when>
			<c:otherwise>
				<li class="page-item">
	...
	<!-- 다음버튼 -->
		...
		<c:when test="${endPage<pageCount }">
			<li class="page-item">
			<a href="${cp}/board/qnalist.jh?pageNum=${endPage+1}&field=${field}&keyword=${keyword}"
